...
from app.services.ai_brain import AIBrain, build_neutral_first_reply, has_raq_project_description
...
def test_first_turn_without_description_uses_neutral_greet(db_session, settings, kb):
    ...
    assert result["reply_text"] == build_neutral_first_reply(conversation)
    assert "How can I help you" in result["reply_text"]
...
def test_first_turn_with_description_uses_smart_fallback(db_session, settings, kb):
    ...
    assert "first_turn" not in result or result.get("first_turn") is not True
